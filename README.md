...
PDF загружается постранично: каждая страница становится отдельным документом с `page_number` в метаданных, поэтому источник можно связать не только с файлом, но и со страницей.
...
Тесты покрывают разбиение текста, загрузку TXT/MD/PDF-страниц, безопасное сохранение upload-файлов, настройки окружения, метаданные чанков, полный путь `build_index -> ask_question`, генераторный fallback, FAISS-поиск, сохранение embedding-модели и поведение пустого индекса.
